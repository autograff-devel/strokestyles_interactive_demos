...
vec2 sdf_mod2(inout vec2 p, vec2 size) {
...
vec2 sdf_mod_mirror2(inout vec2 p, vec2 size) {
...
vec2 sdf_mod_grid2(inout vec2 p, vec2 size) {
	vec2 c = floor((p + size*0.5)/size);
	p = mod(p + size*0.5, size) - size*0.5;
	p *= mod(c,vec2(2))*2 - vec2(1);
	p -= size/2;
...
vec3 sdf_mod3(inout vec3 p, vec3 size) {
...
float sdf_mirror(inout float p, float dist) {
	float s = sign(p);
	p = abs(p)-dist;
	return s;
}
...
float sdf_reflect(inout vec3 p, vec3 planeNormal, float offset) {
...
vec4 trace_ray_enhanced(in vec3 p, in vec3 w, in vec4 bg_clr, in float pixelRadius )
{
    ...
    //if ( candidate_error > pixelRadius ) //t > t_max || candidate_error > pixelRadius )
    //    return bg_clr;
...
vec3 walk_bend_1(in vec3 q, in float t, in float tilt)
{
    ...
    q = sdf_rotate_y(q, radians(90+tilt));
    q = sdf_translate(q, vec3(0.,0.,sin(t*2)*0.035));
    return q;
}

float walking_texture(in vec3 q, in float ex, out int mtl)
{
    float d = 1e10;
    q = walk_bend_1(q, time, 10.);
...
float sin_avg(float x)
{
    ...
    for (int i = 0; i < iter; i++)
        y += sin(x+noise(time*0.1*i)*eps);
    return y/iter;
}
float split_sphere( in vec3 p, float r, int iter, float t )
{
    vec3 q = p;
    float d = 1e10;
    for (int i = 0; i < iter; i++)
    {
        vec3 pq = mod(q, 23);//floor(q*2);

        vec3 q1 = p + vec3(sin_avg(pq.x+t)*2, sin_avg(pq.y + t*2.), sin_avg(t*0.1));//noise(pq.x + t*0.7), noise(pq.z+t), noise(pq.z))*0.3;//sdf_translate(q, vec3(noise(pq.x + t*0.7), noise(pq.z+t), noise(pq.z))*0.3);
        vec3 q2 = p;//sdf_translate(q, vec3(noise(-pq.x*3. + t), noise(pq.z*2.), -noise(pq.z))*0.3);

        float d1 = sdf_box(q1, vec3(r));
        float d2 = sdf_box(q2, vec3(r));
        //return d1;
        vec3 plane = normalize(vec3(noise(pq.x+t)*3, noise(pq.y-t*0.3)*10, noise(-pq.z)));
    ...
    return max(d, sdf_box(p, vec3(1.6)));
    return d; 
}

float sdf_box(in vec2 p, in vec2 size)
{
    vec2 d = abs(p)-size;
...
float compute_scene_menger( in vec3 p, out int mtl )
{
    mtl = 0;
    
    float min_d = 1e10;
    //float d = sdf_box(p, vec3(1.0));//walking_texture(sdf_scale(p, vec3(2.)), 2., mtl); //sdf_sphere(p, 1.);//vec3(1.));
    float d = sdf_sphere(p, 1.);

    //return sdf_cross(p*5)/5;
    float thin = 3.5;//2. + (sin(time*0.1)*0.5+0.5)*2;// mod(time*0.1, 3.);//2.6;
    vec3 q = p;
    float s = 1.0;
    for( int m=0; m<4; m++ )
    {
        q = sdf_scale(q, vec3(1.3));
        q = sdf_translate(q, vec3(0.5));
        
        q = sdf_rotate_y(q, radians(time));
        q = sdf_rotate_z(q, radians(time*1.3));
        
        vec3 a = mod( q*s, 2.0 )-1.0;
        s *= thin;
        vec3 r = 1. - abs(a)*thin;//1.0 - thin*abs(a);
    ...
    min_d = d;
    //min_d = sdf_intersect(min_d, walking_texture(sdf_scale(p, vec3(2.)), 2., mtl));

    d = sdf_xy_plane(p, -1.35);
    set_material(d, min_d, 2, mtl);
    min_d = min(d, min_d);

    //return 
    return min_d;//vec4(d,1.0,1.0,1.0);
}

float compute_scene_splitter( in vec3 p, out int mtl )
{
    mtl = 0;
    ...
    float d = -1e10;
    //return d;//
    //d = sdf_sphere(p, 1.); //sdf_box(p, vec3(1.0));//
    //return  max(d, -sdf_box(sdf_rotate_y(sdf_rotate_x(p, 45), 30), vec3(0.1,3.1,3.)));
    d = sdf_box(p, vec3(1.0));
    //d = walking_texture(sdf_scale(p, vec3(3.)), 0.2, mtl); //sdf_sphere(p, 1.);//vec3(1.));
    //return sdf_cross(p*5)/5;
    float thin = 3.5;//2. + (sin(time*0.1)*0.5+0.5)*2;// mod(time*0.1, 3.);//2.6;
    vec3 q = p;
    float s = 1.0;
    ...
    for( int m=0; m<3; m++ )
    {
        ...
        //qq = sdf_translate(qq, qq);//radians(30));
        //d = max(d, -sdf_box(qq, vec3(1.0))); //walking_texture(sdf_scale(q, vec3(2.)), 0.2, mtl)); //sdf_sphere(p, 1.);//vec3(1.));
        //q = twist_y(q, 0.2, 0);//time*2.1);
        //q = sdf_scale(q, vec3(1.3));
        //q = sdf_translate(q, vec3(0.5));
        ...
        //d = max(sdf_)
        //vec3 a = q;// mod( q*s, 2.0 )-1.0;
        //q
        q = sdf_rotate_x(q, radians(time*10));
        q = sdf_rotate_z(q, radians(time*2.3));
        s *= thin;
        vec3 a = mod(q*s, 5.)-1.;
        float c = -sdf_box(a, vec3(5.52/s,19.1,19.));
        d = max( d, c );// dot(a, vec3(1.,0.,0.)));//vec3(sin(time),-cos(time)*0.2,cos(time)))));//p.y );
...
float compute_scene( in vec3 p, out int mtl )
{
    return compute_scene_menger(p, mtl);
}

float compute_scene_o( in vec3 p, out int mtl )
{
    mtl = 0;
    float min_d = 1e10;
    //float blend_k = 1.;
...
//    d = sdf_union(d, sdf_xz_plane(p,  0));
    ...
    float d = 1e10;
    int obj_mtl = 0;
    //d = walking_texture(q, extr, obj_mtl);

    d = split_sphere(q, extr*4, 1, time*0.1);
...
void main(void)
{
    ...
    float pixel_r = sqrt(pow(1. / resolution.x, 2) + pow(1. / resolution.y, 2))/2;
    vec4 clr = trace_ray_enhanced(p, w, background_color, pixel_r);//(0.5/resolution.x)*sqrt(2));
