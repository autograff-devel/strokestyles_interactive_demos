...
void main(void)
{
    ...
    vec4 src_clr = texture2D(tex_src, uv);
    //src_clr.xyz = rgb_to_hcl(src_clr.xyz);
    vec4 dst_clr = texture2D(tex_dst, uv);
    //dst_clr.xyz = rgb_to_hcl(dst_clr.xyz);
    vec4 dst =  src_clr + (dst_clr - src_clr)*amt;
    if (length(dst_clr.xyz - src_clr.xyz) < 1e-5)
      dst = dst_clr;
    //dst.xyz = hcl_to_rgb(dst.xyz);
    dst.w = dst_clr.w;
    gl_FragColor = dst;
}
