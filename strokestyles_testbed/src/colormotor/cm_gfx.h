...
#define GL_MAKERGBA(r, g, b, a)                                  \
  ((unsigned int)((((unsigned char)(a * 255.0f) & 0xff) << 24) | \
                  (((unsigned char)(b * 255.0f) & 0xff) << 16) | \
                  (((unsigned char)(g * 255.0f) & 0xff) << 8) |  \
                  ((unsigned char)(r * 255.0f) & 0xff)))
#define GL_MAKERGBA255(r, g, b, a)                    \
  ((unsigned int)((((unsigned char)(a)&0xff) << 24) | \
                  (((unsigned char)(r)&0xff) << 16) | \
...
struct Box {
 public:
  ...
  Box(const arma::mat& P) {
    for (int i = 0; i < P.n_cols; i++) includeAt(i, P.col(i));
  }

  Box(const std::vector<arma::mat>& S) {
    int k = 0;
    for (int j = 0; j < S.size(); j++)
      for (int i = 0; i < S[j].n_cols; i++) includeAt(k++, S[j].col(i));
  }

  arma::vec ltrb() const { return arma::vec({l(), t(), r(), b()}); }
  void      ltrb(double l, double t, double r, double b) {
    minmax(0, 0) = l;
    minmax(1, 0) = t;
    minmax(0, 1) = r;
    minmax(1, 1) = b;
  ...
  bool contains(const V2& p) const {
    return (p.x >= min()[0] && p.y >= min()[1] && p.x <= max()[0] &&
            p.y <= max()[1]);
  }
...
struct Contour {
  ...
  Contour(const std::vector<V2>& ptList, bool closed = false) : closed(closed) {
    points = arma::zeros(2, ptList.size());  //,2);
    for (int i = 0; i < ptList.size(); i++) points.col(i) = ptList[i];
  }

  Contour(const std::vector<V3>& ptList, bool closed = false) : closed(closed) {
  ...
  Contour(const arma::mat& mat, bool closed = false) : closed(closed) {
    points = mat;
  }
  ...
  /// Save points to file of type @type, if @savePointsAsRows is true (default)
  /// the contour will be saved transposed with respect to the armadillo matrix
  /// layout
  void save(const std::string& path, arma::file_type type = arma::csv_ascii, bool savePointsAsRows = true);

  /// Load points from file of type @type, if @savePointsAsRows is true
  /// (default) the contour is assumed to be saved transposed with respect to
  /// the armadillo matrix layout
...
struct GLObj {
  ...
  GLObj(GLuint glId, TYPE type) : glId(glId), type(type) {}
...
class Texture : public GfxObject {
 public:
  ...
  enum LOCKFLAGS { LOCK_READ      = 1,
                   LOCK_WRITE     = 2,
                   LOCK_READWRITE = 3 };

  enum WRAP { CLAMP_TO_EDGE = 0,
              CLAMP,
              REPEAT,
              NUM_WRAP_TYPES };
 ...
 public:
  ...
  void setGLData(int w, int h, int hwW, int hwH, GLuint glTarget, GLuint glId, GLint glFormat);

  void createFromGL(int w, int h, GLuint glTarget, GLuint glId, GLint glFormat);
...
class RenderTarget : public GfxObject {
 public:
  RenderTarget();
  RenderTarget(int w, int h, int pixelFormat = DEFAULT_COLOR_FMT, bool depth = true, int depthFormat = DEFAULT_DEPTH_FMT, int numSamples = 0);
  ...
  bool createMrt(int w, int h, std::vector<int> formats, bool hasDepthStencil = true, int depthStencilFormat = DEFAULT_DEPTH_FMT, int numSamples = 0);

  bool create2d(int w, int h, int pixelFormat = DEFAULT_COLOR_FMT, bool depth = true, int depthFormat = DEFAULT_DEPTH_FMT, int numSamples = 0) {
    std::vector<int> fmts;
 ...
 protected:
  GLuint createAndAttachRenderbuffer(GLenum internalFormat,
...
class Image {
 public:
  ...
  /// Update from armadillo cube
  void update(const arma::uchar_cube& src);
  ...
  arma::vec getColor(int x, int y) const;
...
/// Fill modes
enum FILLMODE { FILL_SOLID = 0,
                FILL_WIRE  = 1,
                FILL_POINT = 2,
                NUMFILLMODES };

/// Cull modes
enum CULLMODE { CULL_NONE = 0,
                CULL_CW,
                CULL_CCW,
                NUMCULLMODES };
...
struct Mesh {
  ...
  void vertex(const arma::vec& v) {
    for (int i = 0; i < v.size(); i++) vertices.push_back((float)v[i]);
    if (v.size() < 3) {
  ...
  void normal(const arma::vec& n) {
    for (int i = 0; i < 3; i++) vertices.push_back((float)n[i]);
  }
  ...
  void uv(const V2& coords) { uvs.push_back(_float2(coords.x, coords.y)); }
  void uv(double u, double v) { uvs.push_back(_float2(u, v)); }
  ...
  arma::mat getColors() const {
    ...
    for (int i = 0; i < size; i++) {
      V.col(i) =
          (arma::vec)V4(colors[i].x, colors[i].y, colors[i].z, colors[i].w);
    }
    return V;
  }
...
void setStencilOp(int failFront, int zfailFront, int zpassFront, int failBack, int zfailBack, int zpassBack);
