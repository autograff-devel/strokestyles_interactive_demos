...
std::string convertNsString(NSString *str) { return std::string([str UTF8String]); }
...
void alert(const char *pFormat, ...) {
  ...
  NSAlert *a =
      [NSAlert alertWithMessageText:@"ALERT"  //[NSString alloc] initWithUTF8String:alertText
...
bool alertYesNo(const char *pFormat, ...) {
  ...
  NSAlert *a =
      [NSAlert alertWithMessageText:@"COLORMOTOR:"  //[NSString alloc] initWithUTF8String:alertText
...
void messageBox(const char *pFormat, ...) {
  ...
  NSAlert *a =
      [NSAlert alertWithMessageText:@"Message"  //[NSString alloc] initWithUTF8String:alertText
...
bool openFileDialog(std::string &path, const char *type, int maxsize) {
  ...
  @autoreleasepool {
    ...
    if (strcmp(type, "*") == 0) {
      ...
      NSArray *fileTypes =
          [NSArray arrayWithObjects:nsType, nil];  //, @"gif",@"png", @"psd", @"tga", nil];
...
enum { NUM_EVENT_SLOTS = 1, NUM_EVENT_FDS = 1 };

FileWatcher::FileWatcher(const std::string &path, int msecs) : fileFlags(0), msecs(msecs) {
  /* Open a kernel queue. */
  if ((kq = kqueue()) < 0) {
...
*/
  ...
  vnode_events = NOTE_DELETE | NOTE_WRITE | NOTE_LINK | NOTE_RENAME |
                 NOTE_REVOKE;// | NOTE_ATTRIB;// | NOTE_EXTEND;
  EV_SET(&events_to_monitor[0],
         event_fd,
         EVFILT_VNODE,
         EV_ADD | EV_CLEAR,
         vnode_events,
...
void FileWatcher::run() {
  while (running) {
    int event_count =
        kevent(kq, events_to_monitor, NUM_EVENT_SLOTS, event_data, numFiles, &timeout);
